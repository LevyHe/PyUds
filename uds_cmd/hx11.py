...
from hx11_scripts.CycleTest import CycleBatchTest
...
class GacUdsCom(object):
    ...
    def __init__(self,diag, wake_msg, keygen=None, bus= None,diag_can=None):
        self.wake_msg = wake_msg
        self.diag = diag
        self.keygen = keygen
        self.bus = bus
        self.test_th = None
        self.diag_can = diag_can
        self.diag_fr = diag
    ...
    def UdsCmdProcess(self):
        ...
        subparser.Add_SubArgs('can', help='change default diag to can')
        subparser.Add_SubArgs('fr', help='change default diag to fr')
        ...
        subparser.Add_SubArgs('test', subparser.SubArgObj('value', type=str,choices=['start','stop'], metavar='start or stop'), help='start or stop a test')
    ...
    def _uds_cmd_process(self, diag):
        ...
        if cmds == '':
            ...
            self.diag=self.diag_can
        elif p.cmd == 'fr':
            self.diag=self.diag_fr
        elif p.cmd == 'test':
            if p.value == 'start':
                if not hasattr(self, 'test_obj'):
                    self.test_obj = CycleBatchTest(self.diag_fr, self.wake_msg, self.keygen, self.bus,diag_can=self.diag_can, Verbose=True)
                self.test_obj.test_start()
            elif p.value == 'stop':
                if hasattr(self, 'test_obj'):
                    self.test_obj.test_stop()
        elif p.cmd == 'uds':
            for cmd in p.value:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        wk_path = os.path.join(os.path.dirname(__file__), 'HX11_scripts')
        if not os.path.exists(wk_path):
            bundle_dir = getattr(sys, '_MEIPASS', os.path.abspath(os.path.dirname(__file__)))
            wk_path = os.path.join(bundle_dir, 'HX11_scripts')
        os.chdir(wk_path)
        main(cmds)
